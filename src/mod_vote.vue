	...
	<article class="mod_vote c_txt1">
		...
		<nav class="vote__nav c_txt1">
			<div class="vote__nav_box js_vote_navbox" :style="styleObject">
				<a href="javascript:;" class="vote__nav_item js_vote_nav c_txt3" :class="{cur: item.cur}" data-endtime="2017-05-28 23:50:00"
				   data-end="1" data-index="0" data-voteset="94" v-for="(item,index) in dataArr" @click="clickA(item,index)">{{item.Fvote_title}}</a>
			</div>
		</nav>
			...
			<div class="vote__bd_box js_vote_wrapbox cur" data-index="0" data-voteset="94" style="transform: translateX(0%);">
				<transition-group tag="ul" name="flip-list" class="vote_list js_vote_list" data-voteset="94">
					<li class="js_vote_item vote_list__item vote_list__count--top c_txt3" data-id="94001" data-voteset="94"
					    v-for="item in currArr['Fvote_set']" :key="item.randomNum">
						<a href="javascript:;" class="vote_list__check c_txt1 js_vote_sel " data-id="94001" data-voteset="94"
...
//                    _this.currArr = _this.dataArr[0]['Fvote_set'].slice(0);
		...
		computed: {
			activityStatementMsg() {   //当前选中期的活动是否已经结束判断
				var _this = this;
